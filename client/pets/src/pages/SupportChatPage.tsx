            ...
            <Row className="support-chat-row">
                    ...
                    <Col md={4}>
                        ...
                        <Pagination className="support-pagination">
                            {[...Array(maxPage).keys()].map((index) => (
                                <Pagination.Item className="support-chat-button" key={index} active={index + 1 === currentPage} onClick={() => setCurrentPage(index + 1)}>
                                    {index + 1}
                                </Pagination.Item>
                            ))}
                        </Pagination>
                ...
                <Col md={role === 'support' || role === 'admin' ? 8 : 12}>
                    <Card className="support-card" style={{ height: '70vh' }}>
                        ...
                        <Card.Footer className="support-card-footer">
                            <InputGroup className="support-input-group">
                                <FormControl placeholder="Message" value={newMessage} onChange={handleNewMessageChange} />
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                    <Button className="support-chat-button" variant="primary" onClick={handleSendMessage} disabled={!activeChat}>Send</Button>
                                </div>
                            </InputGroup>
                        </Card.Footer>
                    </Card>
